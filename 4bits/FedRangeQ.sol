...
contract FedRangeQ{
    ...
    struct IndexStru{               // the structure of one index item
        ...
        bytes[8] pubKey;           // the public key of each block [the number of blocks]
        // next line should be changed if bit length changes
        uint8[15][15][8] tag;        // the id of ciphertext with the same tag, the unused items will be filled with 100 (i.e., a value out of the range). e.g. tag[0][1][] includes all the ids in block 0 which their tags are 1 (sub-index) [max number of conflicts in one baskets][the number of baskets][the number of blocks]
        // next line should be changed if bit length changes
        bytes[15][8] ciphertext;    // ciphertext of each block [the number of variables in one block][the number of blocks]
    }
    ...
    struct QueryStru{
        ...
        // next line should be changed if bit length changes
        uint8[8] tag;          // the ciphertext tag of each blocks [the number of blocks]
        // next line should be changed if bit length changes
        bytes[8] ciphertext;   // the ciphertext of each block [the number of blocks]
    }
    ...
    function search(QueryStru memory query) public {
        for(uint i=0;i<index.length;i++){   // scan each item in the index
            ...
            for(uint j=0;j<8;j++){         // scan each block in query
                // next line should be changed if bit length changes
                for(uint k=0;k<15;k++){      // scan the tag list of the current block which their tags are the same as query's
